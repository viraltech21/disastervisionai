...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans overflow-x-hidden">
      <header className="bg-slate-950 border-b border-slate-800 p-6 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-emerald-500/20 text-emerald-400 rounded-lg"><Globe size={32} /></div>
            <div>
              <h1 className="text-2xl font-bold text-slate-100 tracking-tight">AI Climate Guard</h1>
              <p className="text-sm text-slate-400">Unified Predictive Framework for Environmental Monitoring</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-500 uppercase font-semibold tracking-wider">Project Authors</p>
            <p className="text-sm text-slate-300">Shaik Salman & R. Vinith Kumar</p>
            <p className="text-xs text-slate-500">Guide: Mr. David Raju</p>
          </div>
        </div>
      </header>

      <nav className="max-w-7xl mx-auto px-6 py-4 border-b border-slate-800 flex gap-4 overflow-x-auto">
        <button onClick={() => setActiveTab('dashboard')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap ${activeTab === 'dashboard' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}><Activity size={18} /> Global Dashboard</button>
        <button onClick={() => setActiveTab('forecasts')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap ${activeTab === 'forecasts' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}><BarChart3 size={18} /> Predictive Forecasts</button>
        <button onClick={() => setActiveTab('risk')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap ${activeTab === 'risk' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}><ShieldAlert size={18} /> Risk Assessment</button>
